"""
AI-powered transaction categoriser using Claude claude-haiku-4-5.
Batches up to 100 descriptions per API call to minimise cost/latency.
"""
...
def categorise_batch(descriptions: list[str]) -> list[str]:
    """
    Categorise a list of transaction descriptions using Claude.
    Returns a list of category strings in the same order.
    Falls back to "Other" for any failures.
    """
    if not descriptions:
        return []
    ...
    for batch_start in range(0, len(descriptions), batch_size):
        ...
        try:
            ...
            # Keep "Other" fallback for this batch

    return results


def categorise_transactions(descriptions: list[str]) -> list[str]:
    """Public entry point — wraps batch categoriser."""
    return categorise_batch(descriptions)
